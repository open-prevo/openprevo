...
public class JobEndControllerTest extends RestBaseTest {
    ...
    public void getAllJobEndData() throws Exception {
        mockMvc.perform(get("/job-end"))
                ...
                .andExpect(jsonPath("$[0].encryptedOasiNumber", is(Cryptography.hash("756.1335.5778.23"))))
                .andExpect(jsonPath("$[0].retirementFundUid", is("CHE-109.740.084")))
                .andExpect(jsonPath("$[1].encryptedOasiNumber", is(Cryptography.hash("756.3412.8844.97"))))
                .andExpect(jsonPath("$[1].retirementFundUid", is("CHE-109.537.488")))
                .andExpect(jsonPath("$[2].encryptedOasiNumber", is(Cryptography.hash("756.9534.5271.94"))))
